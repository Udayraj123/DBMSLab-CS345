...
int getCredits(char* course_id){
	...
	char* course = (char*)malloc(sizeof(char) * 7);
	int credits;
	while(!feof(fp)){
	...
	fclose(fp);
	// 
	printf("Error: Course credits not found for %s\n", course_id);
	return 999;
}
...
int main(){
	...
	//test getCredits
	char course[7]="BT101"; printf("credits for course %s = %d\n", course, getCredits(course));
	char course2[7]="udayraj"; printf("credits for course2 %s = %d\n", course2, getCredits(course2));
